...
export const api = {
  ...
  async getPageFirstDiscovery(discoveryId:string):Promise<PageFirstDiscovery> {
    ...
    ],areas:[{id:'area-main',kind:'main',name:'전체게임',elementIds:['element-1','element-2']}],interactions:[{id:'interaction-1',areaId:'area-main',elementId:'element-1',kind:'button',name:'#PC 필터',selector:'[data-testid="game-filter-pc"]',enabled:true,risk:'READ_ONLY_CANDIDATE',source:'PAGE_DISCOVERY'}],stateChanges:[{interactionId:'interaction-1',selector:'[data-testid="game-filter-pc"]',before:{url:item.value.pages[0]?.url??'https://staging.storefront.test',ariaPressed:'false',ariaSelected:null,checked:null},after:{url:item.value.pages[0]?.url??'https://staging.storefront.test',ariaPressed:'true',ariaSelected:null,checked:null},source:'PLAYWRIGHT_OBSERVED'}],warnings:[{code:'LIMITED_READ_ONLY_DISCOVERY',message:'안전한 읽기 전용 범위만 분석했습니다. 외부 이동과 위험 동작은 제외됩니다.'}]}
    return structuredClone(item.value)
  },
