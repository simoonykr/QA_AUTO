  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {view === 'page-first' && <PageFirstScenario onToast={toast} onApproved={(approved)=>{if(!approved.versionId)return;setActiveVersionId(approved.versionId);setActiveStructured(toStructuredTestCase(approved));setView('configure')}}/>}
...
function PageFirstScenario({onToast,onApproved}:{onToast:(message:string)=>void;onApproved:(scenario:PageScenarioDraft)=>void}) {
  ...
  const decide=async(id:string,decision:ScenarioDecision)=>{if(!scenario||reviewing)return;const item=comparisons.find(row=>row.id===id);if(!item)return;setReviewing(true);try{const updated=await api.reviewPageScenario(scenario.scenarioId,{expectedRevision:scenario.revision,selections:[{comparisonId:id,decision,draft:item.draft!==item.text?item.draft:undefined}]});setScenario(updated);setMockRevision(updated.revision);setComparisons(updated.comparisons??[]);setEditingComparisonId('');onToast('검토 선택을 서버 revision에 저장했습니다.')}catch(error){await handleScenarioError(error,scenario.scenarioId)}finally{setReviewing(false)}}
  ...
  const approve=async()=>{if(!scenario||pendingCount||approving)return;setApproving(true);try{const updated=await api.approvePageScenario(scenario.scenarioId,{expectedRevision:scenario.revision});setScenario(updated);if(!updated.versionId)throw new Error('versionId missing');onToast('시나리오를 승인했습니다. 실행 설정으로 이동합니다.');onApproved(updated)}catch(error){await handleScenarioError(error,scenario.scenarioId)}finally{setApproving(false)}}
