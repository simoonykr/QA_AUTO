...
type ComparisonResult = 'MATCHED' | 'TC_ONLY' | 'PAGE_ONLY' | 'CONFLICT' | 'NOT_AUTOMATABLE'
type ComparisonDecision = 'PENDING' | 'ADD' | 'MANUAL' | 'EXCLUDE' | 'IGNORE'
type MockComparison = { id:string; result:ComparisonResult; text:string; evidence:string; decision:ComparisonDecision; draft:string }
...
function PageFirstScenario({onToast}:{onToast:(message:string)=>void}) {
  ...
  const [comparisons,setComparisons]=useState<MockComparison[]>([])
  ...
  const generate=async()=>{
    ...
    try{const result=await api.generatePageScenario(discoveryId);if(generation!==requestGeneration.current)return;setScenario(result);setMockRevision(result.revision);setComparisons(createMockComparisons(tcContext,result));onToast('페이지 근거 시나리오 초안을 생성했습니다.')}
  ...
  const updateComparison=(id:string,patch:Partial<MockComparison>)=>setComparisons(items=>items.map(item=>item.id===id?{...item,...patch}:item))
  const decide=(id:string,decision:ComparisonDecision)=>{updateComparison(id,{decision});setMockRevision(value=>value+1);setEditingComparisonId('');onToast('Mock revision에 검토 선택을 반영했습니다.')}
  const invalidate=()=>{requestGeneration.current+=1;setDiscoveryId('');setDiscovery(null);setScenario(null);setComparisons([]);setEditingComparisonId('');setMockRevision(1)}
  const pendingCount=comparisons.filter(item=>item.decision==='PENDING'&&['TC_ONLY','CONFLICT','NOT_AUTOMATABLE'].includes(item.result)).length
  ...
  return <section className="page page-first-page"><div className="page-heading compact"><div><p className="eyebrow">PAGE-FIRST SCENARIO</p><h1>AI 시나리오 초안</h1><p>실제 페이지에서 검증된 요소로 기본 시나리오를 만들고 자연어 TC를 보강 자료로 비교합니다.</p></div><span className="phase-badge">PHASE 1 · AI 0회</span></div>
    ...
    {scenario&&<article className="panel scenario-review"><div className="panel-head"><div><h2>3. 시나리오 비교·편집</h2><p>{scenario.purpose} · revision {mockRevision} <span className="mock-revision">화면 Mock</span></p></div><div className="review-state"><span className={`pill ${pendingCount?'review':'pass'}`}>{pendingCount?`미결정 ${pendingCount}건`:'검토 완료'}</span><button className="primary" disabled title="백엔드 승인 계약 연결 후 활성화">승인·실행 연결 대기</button></div></div><div className="scenario-columns"><div><h3>페이지 근거 단계</h3>{scenario.steps.map((step,index)=><div className="scenario-step" key={step.id}><span>{index+1}</span><div><b>{step.targetDescription}</b><code>{step.selector}</code><small><em>PAGE DISCOVERY</em> · {step.assertion.operator} · {step.evidence.observed}</small></div></div>)}<button className="secondary add-manual" disabled><Plus/>수동 단계 추가 · API 대기</button></div><div><h3>TC 비교 및 보강</h3>{comparisons.length?comparisons.map(item=><div className={`comparison-card ${item.result.toLowerCase()} ${item.decision!=='PENDING'?'decided':''}`} key={item.id}><div className="comparison-title"><b>{item.result.replace('_',' ')} · {comparisonLabel(item.result)}</b>{item.decision!=='PENDING'&&<span>{decisionLabel(item.decision)}</span>}</div>{editingComparisonId===item.id?<textarea value={item.draft} onChange={event=>updateComparison(item.id,{draft:event.target.value})}/>:<p>{item.draft}</p>}<small>{item.evidence}</small><div className="comparison-actions">{editingComparisonId===item.id?<><button className="secondary" onClick={()=>decide(item.id,'ADD')}><Save/>수정 저장</button><button className="secondary" onClick={()=>{updateComparison(item.id,{draft:item.text});setEditingComparisonId('')}}>취소</button></>:<><button className="secondary" onClick={()=>decide(item.id,'ADD')} disabled={item.result==='NOT_AUTOMATABLE'}>시나리오에 추가</button><button className="secondary" onClick={()=>decide(item.id,'MANUAL')}>수동 검증</button><button className="secondary" onClick={()=>setEditingComparisonId(item.id)}>문구 수정</button><button className="secondary" onClick={()=>decide(item.id,'EXCLUDE')}>제외</button></>}</div></div>):<div className="empty-table">TC를 입력하면 누락·불일치 제안을 이 영역에서 검토합니다.</div>}<div className="mock-contract-note"><AlertTriangle/><span>선택 결과는 화면에서만 재현됩니다. 비교·revision API가 연결되기 전에는 서버 저장·승인·실행되지 않습니다.</span></div>{scenario.warnings.map(item=><div className="config-error" key={item.code}><AlertTriangle/><div><b>{item.code}</b><span>{item.message}</span></div></div>)}</div></div></article>}
  </section>
}

function createMockComparisons(rawText:string,scenario:PageScenarioDraft):MockComparison[] {
  const items=rawText.split(/\r?\n|(?<=[.!?])\s+/).map(value=>value.replace(/^[-*\d.)\s]+/,'').trim()).filter(Boolean).slice(0,5)
  if(!items.length)return []
  return items.map((text,index)=>{
    const manual=/육안|디자인|자연스럽|정상적|적절|품질/.test(text)
    const matched=scenario.steps.some(step=>text.includes(step.targetDescription)||step.targetDescription.includes(text))
    const result:ComparisonResult=manual?'NOT_AUTOMATABLE':matched?'MATCHED':index===0?'CONFLICT':'TC_ONLY'
    return {id:`comparison-${index+1}`,result,text,draft:text,decision:result==='MATCHED'?'IGNORE':'PENDING',evidence:result==='MATCHED'?'페이지 분석 단계와 동일한 대상을 확인했습니다.':result==='NOT_AUTOMATABLE'?'자동 판정 근거가 없어 QA 확인이 필요합니다.':result==='CONFLICT'?'페이지 근거와 TC 표현의 대상이 일치하는지 확인하세요.':'페이지 시나리오에 없는 TC 요구사항입니다.'}
  })
}
function comparisonLabel(result:ComparisonResult) { return ({MATCHED:'일치',TC_ONLY:'TC에만 있음',PAGE_ONLY:'페이지에만 있음',CONFLICT:'불일치',NOT_AUTOMATABLE:'자동화 불가'} as const)[result] }
function decisionLabel(decision:ComparisonDecision) { return ({PENDING:'미결정',ADD:'추가됨',MANUAL:'수동 검증',EXCLUDE:'제외됨',IGNORE:'일치 확인'} as const)[decision] }
...
function Author({stage,setStage,onBack,onRun,onVersion,onStructured,onToast}: {stage:AuthorStage; setStage:(s:AuthorStage)=>void; onBack:()=>void; onRun:()=>void; onVersion:(id:string|null)=>void; onStructured:(value:StructuredTestCase|null)=>void; onToast:(s:string)=>void}) {
  ...
  const selectImported = (item:ImportedTestCaseItem) => {setSelectedImportedId(item.externalId??'');setTitle(item.title);setRaw(item.rawText);setStructured(null);onStructured(null);onVersion(null);setStage('draft')}
