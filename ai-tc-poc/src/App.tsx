...
function PageFirstScenario({onToast,onApproved}:{onToast:(message:string)=>void;onApproved:(scenario:PageScenarioDraft)=>void}) {
  ...
  const [candidateApplyingId,setCandidateApplyingId]=useState('')
  ...
  const start=async()=>{
    ...
    try{const result=await api.startPageFirstDiscovery({environmentId,startUrl:startUrl.trim(),includeInternalLinks,maxDepth,maxPages,maxAiCalls:0});if(generation!==requestGeneration.current)return;setDiscovery({discoveryId:result.discoveryId,status:'QUEUED',pages:[],elements:[],warnings:[],errorCode:null,scope:{includeInternalLinks,maxDepth,maxPages},aiUsage:{source:'RULE_BASED',callCount:0}});setDiscoveryId(result.discoveryId);onToast(includeInternalLinks?`내부 링크를 최대 ${maxPages}페이지까지 분석합니다.`:'현재 페이지만 읽기 전용으로 분석합니다.')}
  ...
  const generate=async()=>{
    ...
    if(discovery.elements.length===0)return onToast('페이지에서 실행 가능한 검증 요소를 찾지 못했습니다. 다시 분석해 주세요.')
  ...
  const applyCandidate=async(candidateId:string)=>{if(!scenario||candidateApplyingId)return;const generation=requestGeneration.current;setCandidateApplyingId(candidateId);try{const updated=await api.applyPageScenarioCandidate(scenario.scenarioId,candidateId,{expectedRevision:scenario.revision});if(generation!==requestGeneration.current)return;setScenario(updated);setMockRevision(updated.revision);setComparisons(updated.comparisons??[]);onToast('기능 후보를 실행 시나리오에 추가했습니다. 검토 완료 후 승인해 주세요.')}catch(error){if(generation===requestGeneration.current)await handleScenarioError(error,scenario.scenarioId)}finally{if(generation===requestGeneration.current)setCandidateApplyingId('')}}
  ...
  const approve=async()=>{if(!scenario||pendingCount||executableSelectionCount===0||approving)return;const generation=requestGeneration.current;setApproving(true);try{const updated=await api.approvePageScenario(scenario.scenarioId,{expectedRevision:scenario.revision});if(generation!==requestGeneration.current)return;setScenario(updated);if(!updated.versionId)throw new Error('versionId missing');onToast('시나리오를 승인했습니다. 실행 설정으로 이동합니다.');onApproved(updated)}catch(error){if(generation===requestGeneration.current)await handleScenarioError(error,scenario.scenarioId)}finally{if(generation===requestGeneration.current)setApproving(false)}}
  ...
  const executableSelectionCount=comparisons.filter(item=>item.stepId&&['ADD','IGNORE'].includes(item.decision)).length
  const matchedCount=comparisons.filter(item=>item.result==='MATCHED').length
  const pageOnlyCount=comparisons.filter(item=>item.result==='PAGE_ONLY').length
  const tcOnlyCount=comparisons.filter(item=>item.result==='TC_ONLY').length
  const reviewedCount=comparisons.length-pendingCount
  const discoveryHasElements=Boolean(discovery?.elements.length)
  const discoveredElementCount=discovery?.pages.reduce((sum,page)=>sum+(page.elementCount??0),0)??0
  const discoveredAreas=discovery?.areas??[]
  const discoveredInteractions=discovery?.interactions??[]
  const discoveredStateChanges=discovery?.stateChanges??[]
  const functionalCandidates=scenario?.scenarioCandidates??discovery?.scenarioCandidates??[]
  const functionalCoverage=scenario?.coverage??discovery?.coverage
  const discoveryWarnings=discovery?.warnings??[]
  const busy=starting||generating||reviewing||Boolean(candidateApplyingId)||approving||Boolean(discovery&&!['COMPLETED','FAILED'].includes(discovery.status))
  ...
  return <section className="page page-first-page"><div className="page-heading compact"><div><p className="eyebrow">PAGE-FIRST SCENARIO</p><h1>AI 시나리오 초안</h1><p>실제 페이지에서 검증된 요소로 기본 시나리오를 만들고 자연어 TC를 보강 자료로 비교합니다.</p></div><span className="phase-badge">PHASE 2 · AI 0회</span></div>
    <div className="page-first-grid"><article className="panel page-first-input"><div className="section-head"><div><h2>1. 분석 대상</h2><p>현재 페이지 또는 안전한 내부 링크 범위를 지정합니다.</p></div><MonitorCheck/></div><label className="field-label">실행 환경</label><div className="select-wrap"><select value={environmentId} onChange={e=>{const id=e.target.value;invalidate();setEnvironmentId(id);setStartUrl('')}} disabled={busy}>{environments.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select><ChevronDown/></div><div className="target-context"><b>{selectedEnvironment?.name??'환경 미선택'}</b><span>허용 기준: {selectedEnvironment?.baseUrl??'-'}</span></div><label className="field-label">시작 URL</label><input className="field-input" value={startUrl} onChange={e=>{invalidate();setStartUrl(e.target.value)}} disabled={busy} placeholder="분석할 https:// 주소를 직접 입력하세요."/><label className="field-label">페이지 분석 범위</label><div className="scope-controls"><label><span>탐색 방식</span><select value={includeInternalLinks?'internal':'current'} onChange={event=>{const internal=event.target.value==='internal';invalidate();setIncludeInternalLinks(internal);setMaxDepth(internal?1:0);setMaxPages(internal?3:1)}} disabled={busy}><option value="current">현재 페이지만</option><option value="internal">안전한 내부 링크 포함</option></select></label><label><span>최대 깊이</span><select value={maxDepth} onChange={event=>{invalidate();setMaxDepth(Number(event.target.value))}} disabled={busy||!includeInternalLinks}><option value={0}>0</option><option value={1}>1</option><option value={2}>2</option></select></label><label><span>최대 페이지</span><select value={maxPages} onChange={event=>{invalidate();setMaxPages(Number(event.target.value))}} disabled={busy||!includeInternalLinks}>{[1,2,3,4,5].map(value=><option value={value} key={value}>{value}페이지</option>)}</select></label></div><div className="scope-note"><ShieldCheck/><span>{includeInternalLinks?`허용 도메인 안에서 깊이 ${maxDepth}, 최대 ${maxPages}페이지를 읽기 전용으로 방문합니다.`:'시작 페이지 한 곳만 분석합니다.'} 위험 경로와 중복 URL은 제외됩니다.</span></div><label className="field-label">선택적 자연어 TC</label><textarea className="tc-editor page-first-tc" value={tcContext} onChange={e=>{invalidate();setTcContext(e.target.value)}} disabled={busy} placeholder="선택한 단일 TC의 대상, 행동, 기대 결과를 줄 단위로 입력하세요. 비교 결과는 서버 revision에 저장됩니다."/><div className="privacy-note"><ShieldCheck/><span>GET/HEAD만 허용하며 입력값·쿠키·비밀번호·전체 HTML은 수집하지 않습니다.</span></div>{analysisError&&<div className="config-error persistent-error"><AlertTriangle/><div><b>{analysisError.code}</b><span>{analysisError.message}</span></div></div>}<button type="button" className="ai-button" onClick={()=>void start()} disabled={busy||!environmentId||!startUrl.trim()}>{starting?<Activity className="spin"/>:<Search/>}{starting?'분석 요청 중':'페이지 분석 시작'}</button></article>
      <article className="panel page-first-progress"><div className="section-head"><div><h2>2. 분석 진행</h2><p>페이지 접속부터 시나리오 초안 생성까지 확인합니다.</p></div><span className={`live ${discovery?.status==='COMPLETED'&&discoveryHasElements?'done':discovery?'running':''}`}>{discovery?.status==='COMPLETED'&&!discoveryHasElements?'분석 불충분':discovery?.status??'대기'}</span></div><div className="page-first-stages">{['페이지 접속','요소 수집','후보 검증','시나리오 초안'].map((label,index)=>{const complete=(discovery?.status==='COMPLETED'&&discoveryHasElements&&index<3)||Boolean(scenario);const active=!scenario&&((discovery?.status==='SCANNING'&&index<=1)||(discovery?.status==='COMPLETED'&&index===2));return <div className={complete?'complete':active?'active':''} key={label}><span>{complete?<Check/>:index+1}</span><b>{label}</b></div>})}</div>{discovery?.scope&&<div className="scope-result"><span>방문 <b>{discovery.pages.length}/{discovery.scope.maxPages}페이지</b></span><span>깊이 <b>{discovery.scope.maxDepth}</b></span><span>페이지별 후보 <b>{discoveredElementCount}개</b></span></div>}{discovery?.pages.map(page=><div className="page-summary" key={`${page.depth??0}-${page.fingerprint}`}><ExternalLink/><div><b>{page.title||'제목 없음'}</b><small>{page.url}<br/>깊이 {page.depth??0} · 후보 {page.elementCount??'-'}개 · fingerprint {page.fingerprint.slice(0,16)}</small></div></div>)}{discovery?.status==='COMPLETED'&&<>{(discoveredAreas.length>0||discoveredInteractions.length>0)&&<div className="observation-panel"><div className="observation-head"><div><b>화면 영역·상호작용 근거</b><small>관찰된 영역 {discoveredAreas.length}개 · 상호작용 후보 {discoveredInteractions.length}개 · 상태 변화 {discoveredStateChanges.length}개</small></div><span>관찰 근거</span></div><div className="area-observations">{discoveredAreas.map(area=>{const interactions=discoveredInteractions.filter(item=>item.areaId===area.id);return <div className="area-observation" key={area.id}><div><b>{area.name}</b><small>{area.kind} · 요소 {area.elementIds.length}개</small></div>{interactions.map(item=>{const change=discoveredStateChanges.find(state=>state.interactionId===item.id);const changedFields=change?[change.before.url!==change.after.url?`URL ${change.before.url} → ${change.after.url}`:'',change.before.ariaPressed!==change.after.ariaPressed?`aria-pressed ${change.before.ariaPressed??'-'} → ${change.after.ariaPressed??'-'}`:'',change.before.ariaSelected!==change.after.ariaSelected?`aria-selected ${change.before.ariaSelected??'-'} → ${change.after.ariaSelected??'-'}`:'',change.before.checked!==change.after.checked?`checked ${String(change.before.checked)} → ${String(change.after.checked)}`:'',change.changedAreas?.length?`영역 변화 ${change.changedAreas.map(area=>`${area.name} ${area.beforeItemCount}→${area.afterItemCount}`).join(', ')}`:'',change.restored?'시작 상태 복구 확인':''].filter(Boolean):[];return <div className="interaction-observation" key={item.id}><span>{item.kind}</span><div><b>{item.name}</b><code>{item.selector}</code></div><em>{item.enabled?'사용 가능':'비활성'}</em>{change&&<div className="state-change-evidence"><span>PLAYWRIGHT 관찰</span><b>{changedFields.join(' · ')||'상태 변화가 관찰되었습니다.'}</b></div>}</div>})}{interactions.length===0&&<p>관찰된 상호작용 후보가 없습니다.</p>}</div>})}</div><div className="scope-limit-note"><AlertTriangle/><span>실제로 관찰된 상태 변화는 아래 기능 후보와 TC 커버리지로 표시됩니다. 시작 상태 복구까지 확인된 후보만 Worker 실행 시나리오에 추가할 수 있습니다.</span></div></div>}<div className="element-count"><Database/><span>현재 실행 시나리오 근거 <b>{discovery.elements.length}개</b></span></div>{Boolean(discovery.scope?.includeInternalLinks)&&discovery.pages.length>1&&<div className="scope-limit-note"><AlertTriangle/><span>연결 페이지 후보는 커버리지 참고용입니다. 현재 시나리오 승인과 Worker 실행 근거는 시작 페이지로 제한됩니다.</span></div>}{!discoveryHasElements&&<div className="config-error persistent-error"><AlertTriangle/><div><b>실행 가능한 요소 없음</b><span>페이지에서 검증 요소를 찾지 못했습니다. 실행 환경과 대상 페이지를 확인한 뒤 다시 분석해 주세요.</span></div></div>}<button type="button" className="primary wide" onClick={()=>void generate()} disabled={generating||!discoveryHasElements}>{generating?<Activity className="spin"/>:<WandSparkles/>}{generating?'초안 생성 중':'기본 시나리오 생성'}</button></>}</article></div>
    {discovery?.status==='COMPLETED'&&discoveryWarnings.length>0&&<article className="panel discovery-limit-panel"><div><AlertTriangle/><div><b>분석 범위 및 제외 항목</b><p>현재 결과는 설정한 범위와 안전 정책 안에서 수집됐습니다. 아래 사유는 테스트 커버리지 검토 시 반드시 확인하세요.</p></div><span>{discoveryWarnings.length}건</span></div><ul>{discoveryWarnings.map((warning,index)=><li key={`${warning.code}-${index}`}><code>{warning.code}</code><span>{warning.message}</span></li>)}</ul></article>}
    {discovery?.status==='COMPLETED'&&functionalCandidates.length>0&&<FunctionalCoveragePanel candidates={functionalCandidates} coverage={functionalCoverage} selectedCandidateIds={scenario?.selectedCandidateIds??[]} applyingId={candidateApplyingId} canApply={Boolean(scenario)&&scenario?.status!=='READY'} onApply={candidateId=>void applyCandidate(candidateId)}/>}
    {scenario&&<article className="panel scenario-review"><div className="panel-head"><div><h2>3. 페이지 테스트 커버리지·TC 보강</h2><p>{scenario.purpose} · 서버 revision {mockRevision}</p></div><div className="review-state"><span className={`pill ${pendingCount||executableSelectionCount===0?'review':'pass'}`}>{pendingCount?`미결정 ${pendingCount}건`:executableSelectionCount===0?'실행 단계 없음':'검토 완료'}</span><button className="primary" onClick={()=>void approve()} disabled={Boolean(pendingCount)||executableSelectionCount===0||busy||scenario.status==='READY'} title={executableSelectionCount===0?'시나리오에 추가한 페이지 근거 단계가 필요합니다.':undefined}>{approving?<Activity className="spin"/>:<ShieldCheck/>}{scenario.status==='READY'?'승인 완료':approving?'승인 중':'승인 후 실행 설정'}</button></div></div><div className="coverage-summary"><div><span>페이지 후보</span><b>{scenario.steps.length}</b><small>검증된 화면 근거</small></div><div className="covered"><span>TC 일치</span><b>{matchedCount}</b><small>기존 TC로 확인</small></div><div className="missing"><span>TC 누락 제안</span><b>{pageOnlyCount}</b><small>추가 검토 필요</small></div><div><span>TC만 존재</span><b>{tcOnlyCount}</b><small>페이지 근거 필요</small></div><div><span>검토 진행</span><b>{reviewedCount}/{comparisons.length}</b><small>revision 즉시 저장</small></div></div>{pageOnlyCount>0&&<div className="coverage-feedback"><Sparkles/><div><b>페이지에는 있지만 TC에 없는 테스트 후보가 {pageOnlyCount}개 있습니다.</b><p>각 제안의 페이지 근거를 확인한 뒤 `TC 보강 초안에 추가`를 누르면 현재 draft revision에 즉시 저장됩니다. 모든 항목 검토 후 바로 실행 설정으로 이동할 수 있습니다.</p></div></div>}{!pendingCount&&executableSelectionCount===0&&<div className="config-error persistent-error"><AlertTriangle/><div><b>실행할 검증 단계가 없습니다.</b><span>페이지 근거 항목을 시나리오에 추가하거나 페이지를 다시 분석해 주세요.</span></div></div>}<div className="scenario-columns"><div><h3>페이지 근거 단계</h3>{scenario.steps.map((step,index)=><div className="scenario-step" key={step.id}><span>{index+1}</span><div><b>{step.targetDescription}</b><code>{step.selector}</code><small><em>PAGE DISCOVERY</em> · {step.assertion.operator} · {step.evidence.observed}</small></div></div>)}</div><div><h3>TC 비교 및 보강 피드백</h3>{comparisons.length?comparisons.map(item=><div className={`comparison-card ${item.result.toLowerCase()} ${item.decision!=='PENDING'?'decided':''}`} key={item.id}><div className="comparison-title"><b>{item.result==='PAGE_ONLY'?'TC 누락 제안':item.result.replace('_',' ')} · {comparisonLabel(item.result)}</b>{item.decision!=='PENDING'&&<span>{decisionLabel(item.decision)}</span>}</div>{editingComparisonId===item.id?<textarea value={item.draft} onChange={event=>updateComparison(item.id,{draft:event.target.value})}/>:<p>{item.draft}</p>}<small>{item.source.replace('_',' ')} · {item.evidence}</small><div className="comparison-actions">{editingComparisonId===item.id?<><button className="secondary" onClick={()=>void decide(item.id,item.result==='MATCHED'||item.result==='PAGE_ONLY'?'ADD':'MANUAL')} disabled={reviewing||!item.draft.trim()}><Save/>수정 저장</button><button className="secondary" onClick={()=>{updateComparison(item.id,{draft:item.text});setEditingComparisonId('')}} disabled={reviewing}>취소</button></>:<><button className={item.result==='PAGE_ONLY'?'primary':'secondary'} onClick={()=>void decide(item.id,'ADD')} disabled={reviewing||!['MATCHED','PAGE_ONLY'].includes(item.result)}>{item.result==='PAGE_ONLY'?'TC 보강 초안에 추가':'시나리오에 추가'}</button><button className="secondary" onClick={()=>void decide(item.id,'MANUAL')} disabled={reviewing}>수동 검증</button><button className="secondary" onClick={()=>setEditingComparisonId(item.id)} disabled={reviewing}>문구 수정</button><button className="secondary" onClick={()=>void decide(item.id,'EXCLUDE')} disabled={reviewing}>제외</button></>}</div></div>):<div className="empty-table">TC 없이 페이지 기본 시나리오만 검토합니다.</div>}<div className="mock-contract-note"><ShieldCheck/><span>검토 선택은 서버 revision에 즉시 저장됩니다. 추가한 페이지 근거만 승인 후 Worker가 테스트하며, 수동·제외 항목은 PASS 범위에 포함하지 않습니다.</span></div>{scenario.warnings.map(item=><div className="config-error" key={item.code}><AlertTriangle/><div><b>{item.code}</b><span>{item.message}</span></div></div>)}</div></div></article>}
  </section>
}
...
function FunctionalCoveragePanel({candidates,coverage,selectedCandidateIds,applyingId,canApply,onApply}:{candidates:PageScenarioCandidate[];coverage?:FunctionalCoverageSummary;selectedCandidateIds:string[];applyingId:string;canApply:boolean;onApply:(candidateId:string)=>void}) {
  const coverageItems=[
    ['COVERED','TC 포함'],['PARTIAL','일부 포함'],['MISSING_IN_TC','TC 누락'],['TC_ONLY','TC만 존재'],['NOT_AUTOMATABLE','자동화 불가'],
  ] as const
  return <article className="panel functional-coverage-panel">
    <div className="panel-head"><div><h2>관찰된 기능 후보·TC 커버리지</h2><p>Playwright가 실제 상태 변화와 시작 상태 복구를 확인한 기능만 자동 실행할 수 있습니다.</p></div><span className={`pill ${candidates.some(item=>item.automationStatus==='AUTOMATABLE')?'pass':'review'}`}>{candidates.some(item=>item.automationStatus==='AUTOMATABLE')?'실행 후보 있음':'수동 검토 필요'}</span></div>
    <div className="functional-coverage-summary">{coverageItems.map(([status,label])=><div className={status.toLowerCase()} key={status}><span>{label}</span><b>{coverage?.[status]??candidates.filter(item=>item.coverage===status).length}</b></div>)}</div>
    <div className="functional-candidate-list">{candidates.map(candidate=>{const selected=selectedCandidateIds.includes(candidate.id);const automatable=candidate.automationStatus==='AUTOMATABLE';return <section className={`functional-candidate ${selected?'selected':''}`} key={candidate.id}><div className="functional-candidate-head"><div><b>{candidate.areaName}</b><p>{candidate.purpose}</p></div><span className={`coverage-status ${candidate.coverage.toLowerCase()}`}>{candidate.coverage.replace(/_/g,' ')}</span></div><ol>{candidate.steps.map((step,index)=><li key={`${candidate.id}-${index}`}><span>{index+1}</span><div><b>{step.action.toUpperCase()}</b><small>{step.action==='click'?step.selector:`관찰 상태 확인: ${step.assertion.changedFields.join(', ')}`}</small></div></li>)}</ol><div className="candidate-evidence"><span>{candidate.source}</span><span>신뢰도 {Math.round(candidate.confidence*100)}%</span><span>근거 {candidate.evidence.elementIds.length}개</span><span className={automatable?'candidate-ready':'candidate-manual'}>{automatable?'상태 복구 확인':'수동 검토 필요'}</span></div><button className={selected?'secondary':'primary'} onClick={()=>onApply(candidate.id)} disabled={!canApply||!automatable||selected||Boolean(applyingId)} title={!canApply?'기본 시나리오를 먼저 생성해 주세요.':!automatable?'시작 상태 복구가 확인되지 않아 자동 실행할 수 없습니다.':undefined}>{applyingId===candidate.id?<Activity className="spin"/>:selected?<Check/>:<Play/>}{selected?'테스트 시나리오에 추가됨':applyingId===candidate.id?'추가 중':'테스트 시나리오에 추가'}</button></section>})}</div>
    <div className="scope-limit-note"><AlertTriangle/><span>`AUTOMATABLE` 후보만 추가할 수 있습니다. 추가 후 최신 revision으로 시나리오를 승인하면 Worker가 클릭과 관찰 상태 assertion을 실행하고 전후 PNG를 저장합니다.</span></div>
  </article>
}
...
function Author({stage,setStage,onBack,onRun,onVersion,onStructured,onToast}: {stage:AuthorStage; setStage:(s:AuthorStage)=>void; onBack:()=>void; onRun:()=>void; onVersion:(id:string|null)=>void; onStructured:(value:StructuredTestCase|null)=>void; onToast:(s:string)=>void}) {
  ...
  const openStepEditor = (step:ExecutionPlanStep) => {
    setEditingStep(step)
    setStepDraft({selector:step.selector??'',url:step.url??'',operator:step.operator??'',expected:typeof step.expected==='string'?step.expected:step.expected?JSON.stringify(step.expected):'',value:step.value==='***'?'':step.value??'',secretRef:step.secretRef??'',assertionType:step.assertionType??''})
    setDirtyFields([])
  }
  ...
  const saveStep = async () => {
    ...
    try {
      ...
      const next:StructuredTestCase=planStep?{...structured,steps:structured.steps.map(item=>item.id===planStep.id?{...item,url:planStep.url,selector:planStep.selector,value:planStep.value,secretRef:planStep.secretRef,operator:planStep.operator,expected:typeof planStep.expected==='string'?planStep.expected:planStep.expected?JSON.stringify(planStep.expected):null,assertionType:planStep.assertionType==='observed_state'?null:planStep.assertionType,timeoutMs:planStep.timeoutMs}:item)}:structured
...
function stepTitle(step: ExecutionStepRun) {
  ...
  return type === 'NAVIGATE' ? '페이지 이동' : type === 'WAIT' ? '페이지 렌더링 대기' : type === 'FILL' ? '값 입력' : type === 'CLICK' ? '요소 클릭' : type === 'ASSERT' ? '화면 검증' : `단계 ${step.stepNo}`
}
...
function RunMonitor({state,execution,details,activeStep,start,stop,onResult}: {state:RunState; execution:Execution|null; details:ExecutionDetails|null; activeStep:number; start:()=>void; stop:()=>void; onResult:()=>void}) {
  ...
  const targetUrl = details?.steps.find(step=>stepAction(step)==='NAVIGATE'&&typeof step.action?.url==='string')?.action?.url as string|undefined
