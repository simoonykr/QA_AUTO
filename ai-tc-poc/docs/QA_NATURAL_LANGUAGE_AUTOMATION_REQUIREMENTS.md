...
## 2026-09-14 페이지 전체 테스트 커버리지·즉시 보강 요구

페이지 분석의 결과는 개별 DOM 요소 목록이 아니라 **기능 영역별 테스트 시나리오와 TC 커버리지 피드백**이어야 한다. 예를 들어 KakaoGames의 메뉴, 배너, 전체게임 필터, 게임 카드, 상세 이동, 외부 링크, 반응형 및 접근성 영역을 식별하고, 표시 확인뿐 아니라 안전한 클릭 전후 상태와 목록 변화까지 테스트 후보로 구성한다.

`모두 분석`은 무제한 크롤링을 뜻하지 않는다. 요청에는 탐색 범위(현재 페이지/내부 링크), 최대 페이지 수·깊이, 허용 action과 위험 행동 정책을 포함하고, 응답에는 분석한 영역 수, 상호작용 수, 생성 시나리오 수 및 제외·미지원 사유를 포함한다.

페이지 시나리오와 기존 TC를 기능 단위로 비교하여 `COVERED | PARTIAL | MISSING_IN_TC | TC_ONLY | NOT_AUTOMATABLE` 커버리지 결과를 제공한다. `MISSING_IN_TC`에는 기능 영역, 제안 목적, 사전 상태, 단계, 기대 결과, 페이지 근거 element ID와 자동화 가능성을 포함한다. QA가 제안을 채택하면 현재 draft의 새 revision에 즉시 저장하고 selector를 Playwright로 재검증한 뒤 승인·실행 설정으로 연결한다. 같은 기능·근거의 중복 제안은 새 TC 행을 계속 만들지 않는다.

AI는 서버가 수집·검증한 element ID와 허용 URL만 사용한다. 결제·삭제·로그아웃·외부 전송 등 위험 행동은 제안만 하고 명시적 승인 없이 실행하지 않는다. 미검증 selector, stale fingerprint, 근거 없는 기대 결과 또는 필수 수동 항목이 있으면 실행을 차단한다.

현재 프론트는 기존 `PAGE_ONLY`를 `TC 누락 제안`으로 표시하고 선택을 revision에 즉시 저장하는 UX를 먼저 제공한다. 실제 기능 영역, 클릭 전후 상태, 다중 페이지 탐색 및 기능 단위 coverage 생성은 백엔드 계약과 Worker 확장이 필요하다.
